...
def readimu(*args):
    ...
    if imu[k].name == 'MAT':
        ...
        if nEpochs != int(nEpochs):
            raise ValueError(
                f"The file {imufile} does not have an expected size. Control the type of IMU or if the file is corrupted.")
        ...
        data[0:7, :] = np.fromfile(fid, dtype=np.dtype(imu[k].DataType), count=7 * nEpochs).reshape((7, nEpochs),
                                                                                                    order='F')
        ...
        if fIMU not in acceptable_frequencies:
            ...
            print(
                f"The current frequency of {fIMU:.2f} Hz does not correspond to the usual frequencies used in the Lab.")
...
def read_imu_timespan(imu_file, start_time, end_time):
    # Read IMU data
    columns = ['time', 'gyro_x', 'gyro_y', 'gyro_z', 'acc_x', 'acc_y', 'acc_z']
    full_data = readimu(imu_file, "IXSEA")
    df_full_data = pd.DataFrame(full_data, columns=columns)

    # Select the data corresponding to the right time
    df_data = df_full_data[df_full_data['time'].between(start_time, end_time)]

    # Split data into singular variable per serie
    time_ = df_data.time
    gyro_x = df_data.gyro_x
    gyro_y = df_data.gyro_y
    gyro_z = df_data.gyro_z
    acc_x = df_data.acc_x
    acc_y = df_data.acc_y
    acc_z = df_data.acc_z

    print("\n")
    return time_, gyro_x, gyro_y, gyro_z, acc_x, acc_y, acc_z
